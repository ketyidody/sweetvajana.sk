...
<body style="font-family: Arial, sans-serif; font-size: 14px; color: #111; margin: 0; padding: 0;">
  <div style="max-width: 600px; margin: 40px auto; padding: 0 20px;">
    ...
    <p style="margin: 0 0 24px; color: #555;">{{ __('email.invoice_received') }}</p>

    <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
      <tr style="background: #f5f5f5;">
        <td style="padding: 10px 14px; font-weight: bold;">{{ __('email.order_number') }}</td>
        <td style="padding: 10px 14px;">{{ $order->order_number }}</td>
      </tr>
      <tr>
        <td style="padding: 10px 14px; font-weight: bold;">{{ __('email.date') }}</td>
        <td style="padding: 10px 14px;">{{ $order->created_at->format('d.m.Y') }}</td>
      </tr>
      <tr style="background: #f5f5f5;">
        <td style="padding: 10px 14px; font-weight: bold;">{{ __('email.total') }}</td>
        <td style="padding: 10px 14px;">€{{ number_format($order->total, 2, '.', '') }}</td>
      </tr>
    </table>

    <p style="color: #555;">{{ __('email.thank_you') }}</p>
